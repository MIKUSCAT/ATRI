...
│  ├─ src/routes/diary.ts                      # 日记查询/列表/索引（不再提供手动生成接口）
...
│  ├─ src/services/openai-service.ts           # 统一封装 /chat/completions 调用（支持 env.DEFAULT_MODEL）
│  ├─ src/services/memory-service.ts           # Embedding + Vectorize（日记向量用 summary+highlights）
│  ├─ src/services/data-service.ts             # D1 操作（日志、日记）
│  ├─ src/services/diary-generator.ts          # 调用提示词生成日记（模型取前端/DEFAULT_MODEL）
...
- **Cloudflare Worker `worker/`**：TypeScript + itty-router，`src/index.ts` 注册 `/chat`、`/diary`、`/conversation`、`/media`、`/admin`，并在 `scheduled` 钩子执行 `runDiaryCron()`。
  - `routes/chat.ts` 并行获取工作记忆（当天对话）与向量检索结果，日记 topK=2、其他记忆 topK=3，命中日记会回放当天对话或日记正文；再调用 `callChatCompletions()`，用 `pipeChatStream()` 输出 reasoning/text SSE。
  - `routes/diary.ts` 提供查询/列表/手动索引；日记生成仅由 Cron 驱动。`jobs/diary-cron.ts` 调用 `generateDiaryFromConversation()` 批量补齐日记。
  ...
  - `services/` 拆分 D1、记忆、OpenAI、提示词、日记生成；日记向量使用 summary+highlights；`db/schema.sql` 定义所有表；`wrangler.toml` 预设 R2/Vectorize/D1 绑定与 Cron。
...
2. Worker `/chat`：
   - 并行启动：A）当天对话查询（`fetchConversationLogsSince()` → `buildWorkingMemoryTimeline()`），B）向量检索（`searchMemories()`，日记单独 topK=2，其他 topK=3，合计 6）。
   - 使用 `composeSystemPrompt()` 拼接基础人格、阶段提示、关联记忆，注入工作记忆与日记回放（命中日记会优先拉当天对话，不行则用日记正文/摘要）。
   - 把历史消息与附件格式化为 OpenAI 兼容结构，图片会转换为 `image_url`，普通文件以“用户上传的文件 … 地址 …”的文本说明。
   - 通过 `callChatCompletions()` 调 Cloudflare 代理的 `OPENAI_API_URL`，模型选择顺序：请求 `modelKey` → 环境变量 `DEFAULT_MODEL` → 默认 `openai.gpt-5-chat`，启用 `stream: true`，聊天超时 120s。
...
1. 自动（Cron）：
   ...
   - 定时触发 `runDiaryCron()`：
     ...
     4. 调 `generateDiaryFromConversation()`（模型同样取请求/默认模型），成功后写入 D1 + Vectorize，失败则写入一条 status=`error` 的日记以便排查。
2. 手动索引：
   - 如已有日记文本，可直接调用 `/diary/index` 写入 D1 + Vectorize（跳过模型生成）。
...
- `POST /diary/index`：直接把已有日记文本写进数据库 + Vectorize（用于手动补录）。自动生成由 Cron 负责。
