...
export async function getUserState(env: Env, userId: string): Promise<UserStateRecord> {
  ...
  const valence = Number.isFinite(Number(row.valence)) ? Number(row.valence) : 0;
  const arousal = Number.isFinite(Number(row.arousal)) ? Number(row.arousal) : 0;
  const certainty = Number.isFinite(Number(row.certainty)) ? Number(row.certainty) : 0.5;
...
export async function saveUserState(env: Env, state: UserStateRecord) {
  ...
  await env.ATRI_DB.prepare(
    `INSERT INTO user_states
      (user_id, status_label, status_pill_color, status_text_color, status_reason, status_updated_at, valence, arousal, certainty, last_interaction_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
     ON CONFLICT(user_id) DO UPDATE SET
...
export async function updateEmotionalState(env: Env, params: {
  userId: string;
  valence: number;
  arousal: number;
  certainty: number;
  ...
  const next: UserStateRecord = {
    ...current,
    valence: clampUnit(params.valence),
    arousal: clampUnit(params.arousal),
    certainty: clampZeroOne(params.certainty),
    updatedAt: now
  };
  await saveUserState(env, next);
  console.log('[ATRI] emotional state updated', {
    userId: params.userId,
...
function clampZeroOne(value: number) {
...
function normalizeUserState(state: UserStateRecord): UserStateRecord {
  ...
  return {
    ...
    valence: clampUnit(state.valence),
    arousal: clampUnit(state.arousal),
    certainty: clampZeroOne(state.certainty),
