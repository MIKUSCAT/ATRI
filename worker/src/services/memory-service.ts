...
export async function deleteDiaryVectorsForDate(env: Env, userId: string, date: string): Promise<void> {
  const safeUserId = String(userId || '').trim();
  const safeDate = String(date || '').trim();
  if (!safeUserId || !safeDate) return;
  const ids = Array.from({ length: 10 }, (_, i) => `hl:${safeUserId}:${safeDate}:${i}`);
  try {
    await (env as any).VECTORIZE.deleteByIds(ids);
  } catch (e) {
    console.warn('[ATRI] deleteDiaryVectorsForDate failed', { userId: safeUserId, date: safeDate, e });
  }
}
