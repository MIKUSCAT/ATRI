...
import { updateEmotionalState, updateStatusState, getUserState } from './data-service';
...
/**
 * 夜间推导今天结尾的状态 + 三维情绪（valence/arousal/certainty），写回 user_states。
 */
async function consolidateNightlyState(env: Env, params: {
  ...
  const rawValence = clampNumber((parsed as any).valence, -1, 1, 0);
  const rawArousal = clampNumber((parsed as any).arousal, -1, 1, 0);
  const certainty = clampNumber((parsed as any).certainty, 0, 1, 0.5);

  // 情绪惯性：valence/arousal 相对昨日变化不超 ±0.4
  const current = await getUserState(env, params.userId);
  const valence = constrainDrift(rawValence, current.valence, 0.4);
  const arousal = constrainDrift(rawArousal, current.arousal, 0.4);
  ...
  try {
    await updateEmotionalState(env, {
      userId: params.userId,
      valence,
      arousal,
      certainty
    ...
    console.warn('[ATRI] nightly emotional state update failed', { userId: params.userId, err });
  }
...
function constrainDrift(next: number, prev: number, maxAbsDelta: number) {
  if (!Number.isFinite(prev)) return next;
  const diff = next - prev;
  if (diff > maxAbsDelta) return prev + maxAbsDelta;
  if (diff < -maxAbsDelta) return prev - maxAbsDelta;
  return next;
}
